...
def extract_context_pairs(
    corpus_path, dict_pairvocab, window_size, word2index, position, symmetry="false"
):
    dict_contexts = {}
                ...
                for j in range(i + 1, min(i + 1 + window_size, len(linesplit))):
                    ...
                    if tokenj in dict_pairvocab[tokeni]:
                        ...
                        if indexj not in dict_contexts[indexi]:
                            if symmetry == "true":
                                if (
                                    indexj not in dict_contexts
                                    and indexi not in dict_contexts[indexj]
                                ):
                                ...
                                dict_contexts[indexi][indexj] = {}
                        if position == "c":
                            list_iterate = linesplit[i + 1 : j]
                        elif position == "l":
                            list_iterate = linesplit[max(i - 10, 0) : i]
                        else:
                            list_iterate = linesplit[
                                j + 1 : min(j + 11, len(linesplit))
                            ]
                        for token_cooc in list_iterate:
                            if token_cooc in word2index:
                                index_cooc = word2index[token_cooc]
                                if symmetry == "true" and flip == True:
                                    if index_cooc not in dict_contexts[indexi][indexj]:
                                        dict_contexts[indexj][indexi][index_cooc] = 1
                                    else:
                                        dict_contexts[indexj][indexi][index_cooc] += 1
                                else:
                                    if index_cooc not in dict_contexts[indexi][indexj]:
                                        dict_contexts[indexi][indexj][index_cooc] = 1
                                    else:
                                        dict_contexts[indexi][indexj][index_cooc] += 1
...
def print_contexts(dict_contexts, output_folder_path, min_freq_cooc, index2word):
    if not os.path.exists(output_folder_path):
        ...
        print(
            "WARNING: "
            + output_folder_path
            + " already exists! Existing files will be overwritten."
        )
    for index1 in dict_contexts:
        word1 = index2word[index1]
        output_folder_word1 = output_folder_path + "/" + ("%r" % word1)
        if not os.path.exists(output_folder_word1):
            os.makedirs(output_folder_word1)
        else:
            print(
                "WARNING: subfolder "
                + output_folder_word1
        ...
        for index2 in dict_contexts[index1]:
            ...
            if dict_contexts[index1][index2] == {}:
                continue
            txtfile = open(
                output_folder_word1 + "/" + ("%r" % word2) + ".txt",
                "w",
                encoding="utf-8",
            )
            list_sorted = sorted(
                dict_contexts[index1][index2].items(),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-pairvocab",
        "--input_vocab",
        help="Path of the input pair vocabulary file (tab-separated with at least two columns, one pair per line)",
        required=False,
        default="./vocab/pair_vocab_pmi.txt",
    )
    parser.add_argument(
        "-wordfreq",
        "--input_wordfrequency_file",
        help="Path of the frequency dictionary file. If standard vocabulary, you can write ./vocab/word_frequency_all.txt",
        required=False,
        default="false",
    )
    parser.add_argument(
        "-min_freq",
        "--minimum_frequency",
        help="Minimum frequency of words (needed only if word frequency file is not given)",
        required=False,
        default=5,
    )
    parser.add_argument(
        "-pos_contexts",
        "--position_contexts",
        help='Type/s of context to extract ("central" or "all")',
        required=False,
        default="central",
    )
    parser.add_argument(
        "-output_contexts",
        "--output_folder",
        help="Output directory to store contexts (needed if only central)",
        required=False,
        default="./contexts/",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-min_freq_cooc",
        "--minimum_frequency_context",
        help="Minimum frequency of words between word pair: increasing the number can speed up the calculations and reduce memory but we would recommend keeping this number low",
        required=False,
        default=1,
    )
    ...
    if pos_contexts == "central":
        output_folder_path = os.path.realpath(args["output_folder"]) + "/"
    ...
    if wordfreq_path.lower() == "false":
        ...
        word2index, index2word = load_dictfreq_file(wordfreq_path)

    # Retrieve pair and word vocabulary (dictionary)
    dict_pairvocab = get_vocab_fromfile(pairvocab_path, word2index)

    print(
        "Vocabulary loaded. Now extracting contexts...(this can take a few hours depending on the size of the corpus)"
    )

    # Extract and print contexts
    dict_contexts = extract_context_pairs(
        corpus_path, dict_pairvocab, window_size, word2index, "c", symmetry
    )
    if pos_contexts == "central":
        print(
            "Central contexts succesfully extracted. Now printing them in "
            + output_folder_path
            + " ..."
        )
        print_contexts(dict_contexts, output_folder_path, min_freq_cooc, index2word)
        print(
            "\nFinished. All contexts have been already printed in "
            + output_folder_path
        )
        dict_contexts.clear()
    else:
        ...
        print(
            "Central contexts succesfully extracted. Now printing them in "
            + output_folder_path_central
            + " ..."
        )
        print_contexts(
            dict_contexts, output_folder_path_central, min_freq_cooc, index2word
        )
        print(
            "All central contexts have been already printed in "
            + output_folder_path_central
            + "\n"
        )
    ...
    if pos_contexts != "central":
        ...
        dict_contexts_left = extract_context_pairs(
            corpus_path, dict_pairvocab, window_size, word2index, "l"
        )
        output_folder_path_left = os.path.realpath("./left_contexts/")
        print(
            "Left contexts succesfully extracted. Now printing them in "
            + output_folder_path_left
            + " ..."
        )
        print_contexts(
            dict_contexts_left, output_folder_path_left, min_freq_cooc, index2word
        )
        print(
            "All left contexts have been already printed in "
            + output_folder_path_left
            + "\n"
        )
        ...
        dict_contexts_right = extract_context_pairs(
            corpus_path, dict_pairvocab, window_size, word2index, "r"
        )
        output_folder_path_right = os.path.realpath("./right_contexts/")
        print(
            "Right contexts succesfully extracted. Now printing them in "
            + output_folder_path_right
            + " ..."
        )
        print_contexts(
            dict_contexts_right, output_folder_path_right, min_freq_cooc, index2word
        )
        print(
            "FINISHED. All right contexts have been already printed in "
            + output_folder_path_right
        )
