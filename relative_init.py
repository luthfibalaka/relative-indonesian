...
def load_embeddings(embeddings_path):
    print("Loading FastText embeddings:", embeddings_path)
    try:
        model = gensim.models.FastText.load_fasttext_format(embeddings_path)
    except:
        try:
            model = gensim.models.FastText.load(embeddings_path)
        except:
            print("ERROR! Couldn't load FastText embeddings from " + embeddings_path)
            sys.exit("Couldn't load FastText embeddings")

    vocab = model.wv.index_to_key
    dims = model.wv[vocab[0]].shape[0]
    vocab = set(vocab)
...
def relativeinit_fromcontexts_file(
    output_path, input_contexts_path, modelwords, vocabwords, dimwords, norm_vectors
):
    ...
    for in_folder in listing:
        ...
        for in_file in listing_word:
            ...
            pair = word1 + "__" + word2
            pair_cooc_file = open(
                input_folder_word + in_file, encoding="utf-8"
            ).readlines()
            processed = False
    ...
    txtfile.close()
    print(
        "Done computing relative_init relation embeddings, adding first line to file..."
    )
...
def relativeinit_fromcontexts_dict(
    output_path,
    dict_contexts,
    modelwords,
    vocabwords,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-embeddings",
        "--input_embeddings",
        help="Input word embeddings path",
        required=True,
    )
    parser.add_argument(
        "-norm",
        "--normalization",
        help='Output vectors normalized ("true") or not ("false")',
        required=False,
        default="false",
    )
    parser.add_argument(
        "-output",
        "--output_vectors_path",
        help="Output file path to store relation vectors",
        required=False,
        default="./relative_init_vectors.txt",
    )
    parser.add_argument(
        "-contexts",
        "--input_contexts_path",
        help='Path of the input contexts directory. Write "false" if not provided',
        required=False,
        default="false",
    )
    ...
    parser.add_argument(
        "-corpus", "--corpus_file", help="Input corpus path (tokenized)", required=False
    )
    parser.add_argument(
        "-wordfreq",
        "--input_wordfrequency_file",
        help="Path of the frequency dictionary file",
        required=False,
        default="false",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Alpha smoothing factor in the pmi calculation",
        required=False,
        default=1,
    )
    parser.add_argument(
    ...
    if input_contexts_path == "false":
        ...
        if pairvocab_path.lower() == "false":
            print(
                "Done loading word frequency dictionary. Now creating word and pair vocabularies (this can take a couple of hours depending on the size of the corpus)..."
            )
        ...
        print(
            "All central contexts have been already loaded. Now computing relative-init vectors..."
        )
        relativeinit_fromcontexts_dict(
            output_path,
            dict_contexts,
            modelwords,
            vocabwords,
        ...
        relativeinit_fromcontexts_file(
            output_path,
            input_contexts_path,
            modelwords,
            vocabwords,
